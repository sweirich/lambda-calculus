...
Notation "'list_map'" := List.map.

Definition list_ext {A B} {f g : A -> B} :
  (forall x, f x = g x) -> forall xs,  list_map f xs = List.map g xs.
...
Definition list_comp {A B C} {f: A -> B} {g: B -> C} {h} :
  (forall x, (funcomp  g f) x = h x) -> forall xs, List.map g (List.map f xs) = List.map h xs.
Proof.
...
#[export]Hint Rewrite List.in_map_iff : FunctorInstances.
